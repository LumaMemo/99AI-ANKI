...
- 已完成：配额进度条显示/更新修复：对 `quotaBytes/usedBytes` 做数值兜底，避免 `NaN%` 导致不渲染；极小占比提供最小可见宽度；填充色使用主题变量 `--btn-bg-primary`（随浅/深色主题自动变化）。
...
- 已完成：知识库抽屉顶部加入操作按钮：刷新 / 新建文件夹 / 上传 PDF。
- 已完成：文件夹能力（folder CRUD）：新建、重命名、删除（按产品选择：仅允许删除空文件夹）。
- 已完成：PDF 文件能力：上传、重命名（仅改 displayName）、删除、预览（请求 signed-url 并在新标签打开）。
- 已完成：前端请求封装补齐 `PATCH/DELETE` 支持，并扩展 KB API 封装，供 chat 端直接调用。
- 已完成：后端补齐 `PATCH /kb/files/:id`（重命名文件 displayName）以支持前端“重命名 PDF”。

涉及代码文件：

- [chat/src/views/chat/components/sider/KnowledgeBaseReadonly.vue](../chat/src/views/chat/components/sider/KnowledgeBaseReadonly.vue)（改：新增写操作与预览能力）
- [chat/src/api/kb.ts](../chat/src/api/kb.ts)（改：新增 folder CRUD / upload / rename / delete / signed-url API 封装）
- [chat/src/utils/request/index.ts](../chat/src/utils/request/index.ts)（改：新增 `patch()` / `del()` 并支持非 GET 请求的通用 method）
- [service/src/modules/kb/kb.controller.ts](../service/src/modules/kb/kb.controller.ts)（改：新增 `PATCH /kb/files/:id`）
- [service/src/modules/kb/kb.service.ts](../service/src/modules/kb/kb.service.ts)（改：新增 `renameFile()`）
- [service/src/modules/kb/dto/kbFileRename.dto.ts](../service/src/modules/kb/dto/kbFileRename.dto.ts)（增：文件重命名 DTO）

### 自检清单

- 新增依赖：无。
- 旧数据兼容：兼容。无目录/无文件时均以“暂无”展示；配额为 0 时进度为 0；未登录时入口不可用。
- 交互说明：输入/删除确认交互已统一为项目内 Dialog（见 Step 16），不再依赖浏览器原生 `window.prompt/confirm`。

### 测试指令（推荐）

- Chat 编译验证：`cd 99AI-ANKI/chat && pnpm -s build`
- Service 编译验证：`cd 99AI-ANKI/service && pnpm -s build`
- 本地联调：
  - 启动后端：`cd 99AI-ANKI/service && pnpm -s dev`
  - 启动前端：`cd 99AI-ANKI/chat && pnpm -s dev`
  - 登录后点击侧边栏顶部“知识库”，完成“新建文件夹 -> 上传 PDF -> 预览 -> 重命名 -> 删除”全链路。
...
- 已完成：配额不足 / 单文件超限时，后端返回更清晰的错误信息；chat 端上传失败会刷新配额并给出可见提示。
- 已完成：同级重名（文件夹创建/重命名）后端保持 400 并返回固定可读 message；chat 端保留错误提示（toast + 抽屉内 error 文本）。
- 已完成：删除一致性策略调整：
  - 现策略：**COS 删除失败不阻塞**，仍然直接删除 `kb_pdf` 对应数据库记录，并回收 `kb_user_usage.usedBytes`。
  - 为兼容历史数据（之前可能产生 `status=2` 的行），后端仍保留 `POST /api/kb/files/:id/retry-delete`：当前等价于再次执行删除（幂等），不再强制要求 `status=2`。
  - chat 端删除失败会给出明确提示；若遇到历史 `status=2` 行，会提示“再次删除”。

涉及代码文件：

- [service/src/modules/kb/kb.service.ts](../service/src/modules/kb/kb.service.ts)（改：单文件超限返回 413；配额不足提示；删除逻辑改为 COS best-effort + 直接删库；retry-delete 兼容为幂等删除）
- [service/src/modules/kb/kb.controller.ts](../service/src/modules/kb/kb.controller.ts)（改：新增 `POST /kb/files/:id/retry-delete`）
- [service/src/modules/kb/dto/kbFileList.dto.ts](../service/src/modules/kb/dto/kbFileList.dto.ts)（改：列表项新增 `status` 字段）
- [chat/src/api/kb.ts](../chat/src/api/kb.ts)（改：列表类型补齐 `status`；新增 retry-delete API 封装）
- [chat/src/views/chat/components/sider/KnowledgeBaseReadonly.vue](../chat/src/views/chat/components/sider/KnowledgeBaseReadonly.vue)（改：上传/重名/删除失败的可见提示；兼容历史删除中提示）

### 自检清单

- 新增依赖：无。
- 旧数据兼容：兼容。
  - `GET /kb/files` 新增返回字段 `status`，不影响旧调用方；chat 端对缺失 `status` 的行做了默认值兜底（默认按 active=1 处理）。
  - 兼容历史 `status=2`：可通过 delete 或 retry-delete 直接删库完成收尾。

### 测试指令（推荐）

- Service 编译验证：`cd 99AI-ANKI/service && pnpm.cmd -s build`
- Chat 编译验证：`cd 99AI-ANKI/chat && pnpm.cmd -s build`

（可选）手动验证重试删除：

1) 如库中存在历史 `status=2` 的 PDF 行：在 chat 端该行会提示“再次删除”，点击后会调用 retry-delete。
2) retry-delete 当前为幂等：无论 COS 删除成功/失败，都会直接删库并回收用量。
...
## Step 16：chat 端知识库弹窗 UI 统一（输入框 + 删除确认）

### 目标

- 将知识库的“新建/重命名”等输入框交互替换为项目主页同风格弹窗（玻璃拟态 UI），并兼容浅色/深色主题。
- 将知识库的“删除确认/重试删除确认”替换为项目内 Confirm Dialog，避免浏览器原生弹窗风格割裂。

### 本步骤状态

- 已完成：新增 Prompt Dialog（输入弹窗），用于“新建文件夹 / 重命名文件夹 / 重命名 PDF”等场景。
- 已完成：知识库所有删除确认（删除文件夹/删除 PDF/删除中重试）统一改为项目 Confirm Dialog。
- 已完成：弹窗样式完全复用现有主题变量（`--overlay-mask`、`--text-primary`、`--glass-border` 等），无需额外适配即可在浅色/深色主题下保持一致。

### 涉及代码文件

- [chat/src/components/Dialog/Prompt.vue](../chat/src/components/Dialog/Prompt.vue)（新增：输入弹窗，玻璃拟态 UI）
- [chat/src/utils/dialog.ts](../chat/src/utils/dialog.ts)（改：新增 `dialog().prompt()`，并复用现有 `warning()`）
- [chat/src/views/chat/components/sider/KnowledgeBaseReadonly.vue](../chat/src/views/chat/components/sider/KnowledgeBaseReadonly.vue)（改：把 `window.prompt/confirm` 替换为 `dialog().prompt()` / `dialog().warning()`）

### 自检清单

- 新增依赖：无。
- 主题适配：弹窗遮罩、文字、边框均使用 CSS 变量，浅色/深色主题切换后应自动适配。
- 取消行为：点击“取消/遮罩”不会触发失败提示或误操作。

### 测试指令（推荐）

- Chat 编译验证：`cd 99AI-ANKI/chat && pnpm -s build`
- 本地运行验证：`cd 99AI-ANKI/chat && pnpm -s dev`
  - 登录后打开知识库抽屉，依次验证：
    - 新建文件夹：弹出输入框样式与主页一致
    - 重命名文件夹 / 重命名 PDF：弹出输入框样式与主页一致
    - 删除文件夹 / 删除 PDF / 删除中重试：弹出确认框样式与主页一致
    - 切换浅色/深色主题再重复一次，确保无颜色/对比度问题

### 回滚策略

- 前端回滚：将知识库交互改回 `window.prompt/confirm`（不推荐，仅用于临时止血）。
