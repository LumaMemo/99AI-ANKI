...
### Step 6 完成记录（2025-12-22）

- 已新增 Entity：
  - `99AI-ANKI/service/src/modules/noteGen/noteGenJobArtifact.entity.ts`
- 已注册到显式实体列表（否则不会被同步建表）：
  - `99AI-ANKI/service/src/modules/database/database.module.ts`
  - `99AI-ANKI/service/src/modules/database/initDatabase.ts`

基于当前 `99AI-ANKI/service/.env` 的验证命令：

- 编译：`pnpm -C 99AI-ANKI/service build:test`
- 启动触发建表：`pnpm -C 99AI-ANKI/service dev`
- MySQL 验证：
  - `mysql -h 127.0.0.1 -P 3306 -u root -p2323 chatanki -e "SHOW TABLES LIKE 'note_gen_job_artifact'; SHOW INDEX FROM note_gen_job_artifact;"`
...
- 编译：`pnpm -C 99AI-ANKI/service build:test`
- MySQL 执行脚本（CMD / PowerShell 均可）：
  - `mysql -h 127.0.0.1 -P 3306 -u root -p2323 chatanki < 99AI-ANKI/service/src/modules/noteGen/sql/check_note_gen_schema.sql`
...
- 修改/新增文档：
  ...
  - （可选）在 `99AI-ANKI/docs/20251221cx-bj-preview.md` 加一行“schema 已落地”的链接
