...
| **Step 8**   | 长耗时任务稳定性优化         | 解决 1 小时+ 任务的超时、假死与恢复问题   | 模拟长任务，重启后可断点续传 | ✅ 已完成 |
...
针对 PDF 处理可能耗时 1小时以上的特性，从全局视角解决 HTTP 超时、进程假死、任务丢失等稳定性问题。单个任务最长时间设为 2 小时。

### 实施总结

1. **并发控制与配置化**：
   - 在 `api/main.py` 中引入了 `threading.Semaphore`，通过 `.env` 中的 `PDF_TO_ANKI_MAX_CONCURRENT_JOBS` 进行控制（当前设为 `6`），有效防止多个长耗时任务同时运行导致内存溢出（OOM）。
   - 任务超时时间也实现了配置化，通过 `PDF_TO_ANKI_JOB_TIMEOUT_SECONDS` 设置（当前设为 `7200` 秒/2 小时）。
2. **启动自动恢复**：
   - 实现了 `app.on_event("startup")` 钩子。Worker 启动时会自动查询数据库中处于 `processing` 状态的任务，并重新加入执行队列，解决了进程意外退出导致的任务丢失问题。
3. **断点续传增强**：
   - 修改了 `core/pipeline.py`。在执行流水线前，会从数据库查询已成功完成的步骤（`note_gen_job_step_usage` 表），并自动跳过这些步骤（Step 8 除外，以确保产物生成完整），极大提升了恢复效率。
4. **任务超时机制**：
   - 在 `run_actual_pipeline` 循环中增加了实时时间检查。如果任务运行超过配置的超时时间，将抛出 `TimeoutError` 并自动更新数据库状态为 `failed`。
5. **异步线程池优化**：
   - 使用 `ThreadPoolExecutor` 管理后台任务，线程池大小随并发数动态调整（`MAX_CONCURRENT_JOBS + 5`），确保即使在满载情况下，FastAPI 的主事件循环依然能够响应健康检查和进度查询。

### 验证结果

- **并发验证**：同时发起多个任务，观察到只有前 6 个立即开始执行，后续任务进入等待状态，直到前序任务释放信号量。
- **恢复验证**：在任务执行中途强杀 Worker 进程并重启，观察到日志显示 `Found X jobs to recover`，任务从断点处（跳过已完成步骤）继续运行。
- **超时验证**：在 `.env` 中设置短超时时间（如 30s），验证任务在超时后能正确停止并记录错误信息。
- **配置验证**：验证了修改 `.env` 后无需改动代码即可调整并发上限和超时阈值。

### 验证脚本

```bash
# 运行稳定性与并发测试脚本
python src/test_step8_stability.py
```

### 回滚策略

- 还原 `api/main.py` 中的 `startup_event` 和 `job_semaphore`。
- 还原 `core/pipeline.py` 中的步骤跳过逻辑。
