...
- **入口位置**：不放在 Chat Footer 的 pill 工具区；改为“新对话页主容器”的左上角一个独立 App 图标入口。
  - 新对话页主容器指：`glass-card relative overflow-hidden h-full w-full flex flex-col transition-all duration-300 ease-in-out transform` 这一层（在 Chat 主视图容器内）。
- **页面形态**：点击 App 图标进入“笔记生成专用新页面”，该页面与当前对话页分离。
- **自动弹出知识库**：进入专用页后，自动打开左侧知识库侧边栏/抽屉（复用现有 KB Readonly 能力）。
- **范围约束**：笔记生成页只实现“选择 KB PDF → 创建任务 → 轮询进度 → 下载交付/失败续跑”闭环；不与“图表/深度思考/联网”同组。

### 10.2 参数选择与固定配置展示（新设计）

必选输入（确定版）：

- `kbPdfId`：知识库中的一个 PDF 文档（来自 KB 选择）。
- `pageRange`：本期不可修改，固定 `{ mode: 'all' }`。

专用页展示（确定版）：

- 页面展示“当前已选择 PDF 信息”（名称/ID）。
- 页面展示“固定输出类型”：`markdown-markmap` 与 `word`（本期固定两种都生成）。
- 未选择 PDF 时，“开始生成”按钮应阻止提交并 Toast 提示“请先在知识库中选择一个 PDF”，并保持/再次打开知识库侧边栏。

### 10.3 创建任务（在专用页内完成）

提交动作（确定版）：

- 用户在专用页点击“开始生成笔记”后调用 `POST /note-gen/jobs`。
- 成功后在专用页内展示任务状态（不在对话消息流中插入任务卡片消息）。

错误处理（确定版）：

- 若后端返回 400（余额不足门槛）：专用页弹出充值提示。

### 10.4 进度轮询与 6 段进度条（在专用页内完成）

轮询与刷新（确定版）：

- 自动刷新：每 60 秒拉一次 `GET /note-gen/jobs/:jobId`。
- 手动刷新：专用页提供“刷新”按钮。
- 进度展示：展示 6 段进度条（0~100% 数字），不暴露内部步骤含义。

退出重进恢复（确定版）：

- 用户离开专用页/开新对话不影响任务执行；再次进入专用页时，若存在当前 jobId，应能恢复展示并继续轮询。

### 10.5 失败续跑与产物下载（在专用页内完成）

失败/未完成（确定版）：

- `status=failed/incomplete`：展示 `userMessage` 错误信息。
- 操作按钮：显示“继续生成/重试”按钮，点击后再次调用 `POST /note-gen/jobs` 触发幂等续跑（后端断点续传接管）。

完成与下载（确定版）：

- 仅当 `status=completed` 且 `resultFiles` 包含 `markdown-markmap` 与 `word` 且均 `ready` 时展示下载区。
- 点击下载：调用 `GET /note-gen/jobs/:jobId/files/:fileType/signed-url`，拿到 URL 后立即触发浏览器下载。
...
- Chat（99AI-ANKI/chat）：
  ...
  - 新对话页左上角“笔记生成 App 图标”入口 + 笔记生成专用页面（待前端实现）
  - 专用页内：PDF 选择（自动弹出知识库）、创建任务、进度轮询（6 段）、失败续跑、下载按钮（待前端实现）
