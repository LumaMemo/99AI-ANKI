...
```bash
curl -H "Authorization: Bearer TOKEN" http://localhost:3000/api/kb/quota
```
...
1) 创建：
```bash
curl -X POST http://localhost:3000/api/kb/folders \
  -H "Authorization: Bearer TOKEN" \
  -H "Content-Type: application/json" \
  -d "{\"parentId\":0,\"name\":\"work\"}"
```
2) 树：
```bash
curl -H "Authorization: Bearer TOKEN" http://localhost:3000/api/kb/folders/tree
```
3) 删除（空文件夹应成功）：
```bash
curl -X DELETE -H "Authorization: Bearer TOKEN" http://localhost:3000/api/kb/folders/ID
```
...
VALUES (1, 0, 'demo', 'demo.pdf', 'pdf', 'application/pdf', 100, 'b', 'r', 'k', 1, NOW(), NOW(), NOW());
```
- curl：
```bash
curl -H "Authorization: Bearer TOKEN" "http://localhost:3000/api/kb/files?folderId=0&page=1&size=10"
```
...
VALUES
 ('kbTencentCosStatus','1',NOW(),NOW(),NOW()),
 ('kbCosSecretId','<id>',NOW(),NOW(),NOW()),
 ('kbCosSecretKey','<key>',NOW(),NOW(),NOW()),
 ('kbCosBucket','<bucket>',NOW(),NOW(),NOW()),
 ('kbCosRegion','<region>',NOW(),NOW(),NOW()),
 ('kbCosPrefix','kb',NOW(),NOW(),NOW()),
 ('kbCosSignedUrlExpiresSeconds','60',NOW(),NOW(),NOW()),
 ('kbSinglePdfMaxBytes','52428800',NOW(),NOW(),NOW());
```
...
```bash
curl -X POST "http://localhost:3000/api/kb/files/upload?folderId=0" \
  -H "Authorization: Bearer TOKEN" \
  -F "file=@./a.pdf"
```
- 验证 DB：
...
1) 获取文件 id：
```bash
curl -H "Authorization: Bearer TOKEN" "http://localhost:3000/api/kb/files?folderId=0&page=1&size=1"
```
2) 获取 signed-url：
```bash
curl -H "Authorization: Bearer TOKEN" http://localhost:3000/api/kb/files/ID/signed-url
```
...
### 验证脚本
```bash
curl -X DELETE -H "Authorization: Bearer TOKEN" http://localhost:3000/api/kb/files/ID
```
验证 DB：
```sql
