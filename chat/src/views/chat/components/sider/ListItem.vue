...
<template>
  <div class="contents">
    ...
    <div v-for="item of dataSources" :key="`${item.uuid}`">
      <div
        class="relative group flex items-center gap-3 px-3 py-2 break-all rounded-xl cursor-pointer font-medium text-sm bg-transparent border border-transparent transition-[background,border-color,transform] duration-200"
        :class="
          isActive(item.uuid)
            ? [
                'aurora-border',
                'neon-ring',
                'bg-[color:var(--glass-bg-primary)]',
      ...
      >
        <div class="flex items-center">
          <input
            ...
            v-focus
            type="text"
            class="bg-transparent border border-[color:var(--glass-border)] text-[color:var(--text-primary)] px-1 shadow-none flex-1 truncate"
