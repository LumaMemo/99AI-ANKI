...
### Step 11: 整页滚动统一体验（禁止卡片内滚动） (Done ✅)
- **背景问题**:
    - 当 **知识点很多 / 单个知识点很长 / PDF 很多** 时，在 `http://localhost:49002/knowledge-card` 页面：
        - **桌面端**：右侧主内容区域无法向下滚动查看后续内容。
        - **移动端**：知识点内容过长时也无法下滑查看。
    - 现状体验表现为：页面存在滚动“锁死”或滚动被某个容器截断；即便能滚动，也倾向于出现“卡片内部滚动”的模式。

- **目标（强约束）**:
    - 实现 **整个页面下滑滚动**（统一滚动容器），而不是“右侧卡片内滚动”。
    - 左侧目录（文件/节点很多）与右侧内容（卡片很多/很长）遵循同一滚动逻辑：用户只需要滚动一次即可浏览所有可见内容。
    - 兼容移动端抽屉侧边栏：打开/关闭抽屉不应导致页面滚动失效或被锁住。

- **设计方案（布局与滚动策略）**:
    1. **唯一滚动容器**：将滚动行为上移到“页面级容器”（通常是页面根容器或 `body` / 应用内容容器）。
        - 页面允许自然撑高：内容高度由左/右两栏中更高者决定，整体通过页面滚动浏览。
    2. **禁止内部滚动**：左侧目录树容器、右侧内容容器、卡片详情容器不再承担 `overflow-y: auto`/`scroll` 的职责。
        - 若当前使用了 `height: 100vh` + `overflow: hidden` 的组合，需要调整为 `min-height` 思路，避免截断。
    3. **固定头部的取舍（可选）**：
        - 若顶部 Header 需要固定：允许 Header `position: sticky`，但仍保持页面整体可滚动；避免在主内容区再包一层独立滚动。
        - 若不固定：Header 参与页面流，滚动更简单、风险更低。
    4. **移动端抽屉滚动兼容**：
        - 抽屉打开时，允许抽屉自身滚动（仅抽屉内部），但关闭抽屉后页面滚动必须立即恢复。
        - 避免抽屉组件（或遮罩）对 `body` 注入永久 `overflow: hidden` 导致“关闭后仍不可滚动”。

- **实现范围（仅描述，不在本 Step 改代码）**:
    - **页面根布局**：检查知识点卡片视图根节点与其父级（如 `chat.vue` / 布局容器）是否存在 `overflow: hidden`、`position: fixed`、`height: 100vh` 等导致滚动被截断的样式。
    - **右侧内容区**：移除任何“内容区内部滚动”的容器设计（比如固定高度 + overflow）。
    - **左侧目录区**：同样不做独立滚动；目录很长时通过整页滚动浏览（与右侧一致）。
    - **移动端**：验证抽屉开关过程中不会破坏页面滚动（尤其是关闭抽屉后）。

- **验收标准（可操作）**:
    - **桌面端**：
        - 在目录树节点很多、Topic 网格很多、或单个卡片详情很长时：鼠标滚轮/触控板滚动能持续向下查看，页面不会“滚不动”。
        - 不出现“右侧卡片内部滚动条”（除非是 Markdown 代码块等天然需要横向滚动的局部）。
    - **移动端**：
        - 在长内容卡片详情页：手指下滑可以连续浏览到页面底部。
        - 打开抽屉后可滚动目录；关闭抽屉后主页面仍可正常滚动。

- **回滚策略**:
    - 仅涉及布局容器的滚动策略与样式调整；若出现兼容性问题，可恢复到原先的容器滚动方式（右侧/左侧独立滚动）作为短期回退。

- **本次实现（代码落地）**:
    - **页面级统一滚动容器**: 在 `chat.vue` 针对 `KnowledgeCard` 路由启用页面级 `overflow-auto`（避免 `#app { overflow: hidden; }` 导致滚动被截断）。
    - **禁止内部滚动**: 移除知识卡片相关视图/组件中的 `h-full` + `overflow-y-auto/hidden` 等内部滚动约束，让内容自然撑高。
    - **移动端抽屉兼容**: 目录树组件新增 `scrollable` 开关，仅在抽屉内启用目录区域内部滚动；关闭抽屉后页面滚动立即可用。

- **修改代码**:
    - `chat/src/views/chat/chat.vue`
    - `chat/src/views/knowledgeCard/index.vue`
    - `chat/src/views/knowledgeCard/components/KnowledgeTree.vue`
    - `chat/src/views/knowledgeCard/components/TopicGrid.vue`
    - `chat/src/views/knowledgeCard/components/PdfSelector.vue`
    - `chat/src/views/knowledgeCard/components/CardDetail.vue`

- **自检**:
    - 无新增依赖；仅调整布局样式与组件 props。
    - 不影响旧数据/接口；不触碰后端与核心业务状态。
    - 潜在风险：页面整体高度可能变大（预期行为）；若后续希望顶部 Header 固定，可用 `position: sticky`，但不要再引入主内容区独立滚动。

- **验证命令（构建自检）**:
    - `pnpm -C 99AI-ANKI/chat build-check`
    - **成功示例**: `vue-tsc --noEmit` 无报错并且 `vite build` 正常完成（进程退出码 0）。

### Step 12: 回归问题修复（主页存储量不更新 / 移动端入口无响应） (Done ✅)
- **目标**:
    - 修复 Step 11 后出现的两处回归：主页配额显示、移动端笔记生成入口。

- **问题 1：主页知识库左上角存储量一直显示 0**
    - **现象**:
        - 在“主页 -> 知识库”模块，左上角存储量长期显示 `0`。
        - 点击进入知识库页面后，存储量又能正常显示（说明数据并非真的为 0）。
    - **影响范围**:
        - 主页知识库入口的信息准确性（用户误判存储状态）。
    - **疑似原因（待代码确认）**:
        - 主页组件未触发拉取存储量的接口 / store 更新（生命周期未执行或条件分支被改动）。
        - store 中用于展示存储量的字段在主页未订阅/未 watch，进入详情页才更新。
        - 滚动容器调整后，主页某些 onMounted/onActivated 触发条件变化（需要确认是否 keep-alive）。
    - **验收标准**:
        - 进入主页知识库区域即可看到非 0 的正确存储量（与知识库页一致）。
        - 不需要先点进知识库页面才会刷新。

- **问题 2：移动端对话框左上角“笔记生成”图标按钮点击无反应**
    - **现象**:
        - 手机端对话页面左上角的“笔记生成”图标按钮点击后无跳转、无反馈。
    - **影响范围**:
        - 移动端用户无法通过快捷入口进入笔记生成界面。
    - **疑似原因（待代码确认）**:
        - 点击区域被遮罩/容器覆盖（可能与滚动/层级 z-index 调整有关）。
        - 路由跳转被拦截或被 watch 逻辑立刻重定向回聊天页（需要核对 `chat.vue` 的 active group watch）。
        - 移动端条件分支导致按钮未绑定事件或事件被阻止。
    - **验收标准**:
        - 移动端点击该按钮后能稳定进入 `/note-gen`（或对应笔记生成路由）。
        - 点击后至少有明确的视觉反馈（例如页面切换），并且不会被立即跳回。

- **本次实现（代码落地）**:
    - **主页配额**: `KnowledgeBaseReadonly.vue` 在登录后立即拉取一次 `loadQuota()`（仅配额，不加载 tree/files），避免首页长期显示 0。
    - **移动端入口**: `NoteGenAppIcon.vue` 将无效的 `z-8` 改为有效层级 `z-20`，避免被 `Header`/内容层遮挡导致点击无响应。

- **自检**:
    - 无新增依赖；仅调整请求触发时机与一个 z-index class。
    - 不影响知识卡片页（`/knowledge-card`）与现有抽屉加载策略（tree/files 仍仅在打开抽屉时加载）。
...
### 2025-12-27: Step 11 - 整页滚动统一体验（禁止卡片内滚动） (Completed)
- **任务描述**: 修复知识卡片页在桌面端/移动端的滚动锁死与“卡片内滚动”问题，统一为页面级滚动。
- **关键变更**:
    - **滚动容器上移**: `chat.vue` 对 `KnowledgeCard` 路由启用页面级 `overflow-auto`，避免父级 `overflow-hidden` 截断滚动。
    - **移除内部滚动**: `KnowledgeCard` 视图、`TopicGrid`、`PdfSelector`、`CardDetail` 等不再承担滚动（移除 `overflow-y-auto` / `h-full` 组合）。
    - **抽屉滚动可用**: `KnowledgeTree` 新增 `scrollable`，仅在移动端抽屉内启用目录区域滚动。
- **自检**: 无新依赖；知识卡片页以外路由保持原有滚动策略。
- **验证**:
    - 桌面端：目录很长/卡片详情很长时，鼠标滚轮可连续滚动到底部，右侧不出现独立滚动条。
    - 移动端：打开抽屉可滚动目录；关闭抽屉后主页面仍可正常下滑。

### 2025-12-27: Step 12 - 回归问题记录（待修复）
- **任务描述**: 记录 Step 11 后出现的两处回归问题，定义后续修复范围与验收口径。
- **问题清单**:
    - 主页知识库左上角存储量一直为 0（进入知识库页后又正常）。
    - 移动端对话页左上角“笔记生成”图标按钮点击无响应。
- **备注**: 本次仅更新文档，不做代码修改。

### 2025-12-27: Step 12 - 回归问题修复（主页配额 / 移动端入口） (Completed)
- **任务描述**: 修复主页知识库配额显示与移动端“笔记生成”入口点击无响应。
- **关键变更**:
    - **主页配额刷新**: `KnowledgeBaseReadonly.vue` 在登录后自动拉取 `loadQuota()`，确保主页入口展示真实已用/总额。
    - **入口可点击**: `NoteGenAppIcon.vue` 修正无效层级类 `z-8` 为 `z-20`，避免被页面层级遮挡。
- **自检**: 无新增依赖；风险面仅限展示与点击层级。
- **验证**: `pnpm -C 99AI-ANKI/chat build-check`
