...
## 3. 总结与后续建议
- **当前进度**: 核心功能已全部完成 (Step 1 - Step 8)。
- **后续优化**:
    - **导出功能**: 实现卡片导出为 Anki 包或 PDF。
    - **记忆算法**: 集成间隔重复算法，支持在查看器中直接进行复习。
    - **多模态支持**: 增加对图片和音频知识点的渲染支持。
- **目标**: 完善页面间的跳转闭环，增加全局返回按钮。
- **修改内容**:
    - **Header**: 在 `index.vue` 的面包屑左侧增加“返回”按钮。
    - **逻辑控制**: 实现详情页 -> 网格页 -> 根目录 -> 知识库列表的层级回退逻辑。
    - **移动端**: 增加右下角悬浮返回按钮（可选）。
- **验证**: 点击返回按钮能准确回到上一级状态，无逻辑死循环。

### Step 9: 全文搜索与高亮增强
- **目标**: 实现深度内容检索，并在进入卡片后自动高亮关键词。
- **修改内容**:
    - **后端**: 扩展 `getCardTree` 接口，支持对 `content` 字段的模糊匹配。
    - **前端搜索**: 搜索结果展示内容摘要（Snippet）。
    - **渲染器**: `MarkdownRenderer.vue` 增加关键词匹配逻辑，使用 `search-highlight` 类进行视觉标记。
